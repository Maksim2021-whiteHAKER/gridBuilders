...
export function exportToRoblox(objects: SceneObject[]): void {
    // ✅ Начинаем с правильного XML-заголовка
    ...
    for (const obj of objects) {
        if (obj.type === 'text' || obj.type === 'pyramid' || obj.type === 'tor' || obj.type === 'cone') {
            console.warn(`[Export] Тип "${obj.type}" пока пропускается (будет добавлен позже)`);
            continue;
        }
        ...
        if (obj.type === 'sphere') shapeToken = 0; // 0 = Ball
        if (obj.type === 'cylinder') shapeToken = 2; // 2 = Cylinder
        ...
        // ✅ Пуленепробиваемый шаблон свойства
        xml += `  <Item class="Part" referent="RBX${obj.id.slice(0, 4)}">\n`;
        xml += `    <Properties>\n`;
        xml += `      <string name="Name">${obj.type}_${obj.id.slice(0, 4)}</string>\n`;
        ...
        xml += `      <bool name="Anchored">true</bool>\n`; // ✅ КРИТИЧЕСКИ ВАЖНО: чтобы не падало
        xml += `    </Properties>\n`;
        xml += `  </Item>\n`;
    }
    ...
    // ✅ КРИТИЧЕСКИ ВАЖНО: правильная кодировка и расширение .rbxmx (XML Model)
    const blob = new Blob([xml], { type: 'application/xml;charset=utf-8' });
    const url = URL.createObjectURL(blob);
    const link = document.createElement('a');
    link.href = url;
    
    // Расширение ДОЛЖНО быть .rbxmx для текстового формата!
    ...
    console.log(`[Export] Файл .rbxmx успешно сгенерирован! 🚀`);
}